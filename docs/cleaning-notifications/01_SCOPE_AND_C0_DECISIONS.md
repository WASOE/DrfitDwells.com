...
| **C6** | Rules + templates seed | 2 triggers × 2 propertyScopes × channels; 8 WA + 8 email template rows (draft); rules `enabled:false`, `mode:'shadow'`, `whatsapp_first_email_fallback`; insert-only seed; add cleaner `ruleKey`s to `PREVIEW_RULE_KEYS` and remove C5 temporary `audience:'cleaner'` preview allowlist bypass |
