...
function looksLikeValleyStay(cabin) {
  const blob = [cabin?.location, cabin?.name, cabin?.slug, cabin?.propertyKind]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
  return /valley|chereshovo|ortsevo|a-?frame|lux/.test(blob);
}
...
export default function MapArrival({ cabin }) {
  ...
  const location = cabin?.location || 'Bulgaria';
  const isValley = looksLikeValleyStay(cabin);

  const routeHint = useMemo(() => {
    if (!isValley) return null;
    return `${formatRouteArrowLine()} → park at ${CHERESHOVO_PARKING.coordinatesDisplay}`;
  }, [isValley]);

  const finalApproachNote = isValley
    ? `From parking: ${FINAL_APPROACH.distancePhrase} (~${FINAL_APPROACH.walkMinutesApprox} min walking) by ${FINAL_APPROACH.modesPhrase}. Do not drive a normal car beyond parking.`
    : null;
        ...
        <div className="mb-4 bg-gray-50 rounded-xl p-4 border border-gray-200">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              {finalApproachNote ? (
                <p className="text-xs text-gray-500 mt-2 italic">{finalApproachNote}</p>
              ) : null}
            </div>
          </div>
        </div>
        ...
        {hasGeoLocation ? (
          <div className="relative aspect-[16/9] bg-gray-100">
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
              <div className="text-center p-8">
                ...
                <p className="text-xs text-gray-400 mt-2 italic">Exact location shared after booking</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="aspect-[16/9] bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
            <div className="text-center p-8">
              ...
              <p className="text-sm text-gray-600 font-medium">{location}</p>
              <p className="text-xs text-gray-500 mt-2">Map coordinates will be available after booking</p>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              {cabin.meetingPoint.what3words && (
                <p className="text-xs text-blue-600 mt-1 font-mono">{cabin.meetingPoint.what3words}</p>
              )}
            </div>
          </div>
        </div>
