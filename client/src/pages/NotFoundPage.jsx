...
const withLocale = (language, path) => {
  if (language !== 'bg') return path;
  if (path === '/') return '/bg';
  return `/bg${path}`;
};
...
const NotFoundPage = () => {
  ...
  return (
    <>
      <Seo title={tc('notFound.seoTitle')} description={tc('notFound.seoDescription')} suppressCanonical noindex />
      ...
      <main className="relative min-h-[calc(100vh-var(--header-offset))] bg-[#F8F5EF] text-[#1F1B17] overflow-hidden">
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(180deg, transparent 0%, rgba(31,27,23,0.85) 100%)'
          }}
        />
        ...
        <section className="relative max-w-7xl mx-auto px-4 sm:px-6 py-10 md:py-16 lg:py-20">
          <div className="max-w-4xl lg:max-w-[620px] lg:ml-auto lg:mr-[8%]">
            <p className="text-[11px] uppercase tracking-[0.32em] text-[#5B554D]">{tc('notFound.kicker')}</p>
            ...
            <p className="mt-5 text-base sm:text-lg text-[#49433B] max-w-2xl">{tc('notFound.lead')}</p>
            <p className="mt-2 text-sm sm:text-base text-[#5E574D]">{tc('notFound.hint')}</p>
            ...
            <div className="mt-8 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              ...
              >
                {tc('notFound.ctaValley')}
              </Link>
            </div>

            <div className="mt-4">
              <Link
                to={withLocale(language, '/search')}
