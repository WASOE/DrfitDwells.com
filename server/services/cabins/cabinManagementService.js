...
const createCabinFromAdminPayload = async (body, ctx = {}) => {
  const allowMulti = featureFlags.isMultiUnitGloballyEnabled();
  const { sanitized, errors } = sanitizeCabinPayload(body, { isUpdate: false, allowMulti });

  if (errors.length) {
    return { ok: false, status: 400, payload: { success: false, message: 'Validation failed', errors } };
  }
  ...
  if (inventoryMode === 'single') {
    ...
    return {
      ok: true,
      status: 201,
      payload: { success: true, message: 'Cabin created successfully', data: { cabin: responseCabin } }
    };
  }
...
const updateCabinFromAdminPayload = async (id, body, ctx = {}) => {
  const syncMultiUnitGalleryToCabinType = ctx.syncMultiUnitGalleryToCabinType;

  const cabin = await Cabin.findById(id);
  if (!cabin) {
    return { ok: false, status: 404, payload: { success: false, message: 'Cabin not found' } };
  }

  const allowMulti = featureFlags.isMultiUnitGloballyEnabled();
  const existingInventoryMode = cabin.inventoryMode || cabin.inventoryType || 'single';
  const incomingInventoryMode =
    body.inventoryMode !== undefined && body.inventoryMode !== null
      ? String(body.inventoryMode).toLowerCase()
      : undefined;
  const incomingInventoryType =
    body.inventoryType !== undefined && body.inventoryType !== null
      ? String(body.inventoryType).toLowerCase()
      : undefined;
  ...
  if (requestedInventoryMode === 'multi' && !allowMulti) {
    return {
      ok: false,
      status: 400,
      payload: { success: false, message: 'Multi-unit cabins are currently disabled' }
    };
  }
  ...
  const unitsProvided = body.units !== undefined;
  ...
  if (collectedErrors.length > 0) {
    return {
      ok: false,
      status: 400,
      payload: { success: false, message: 'Validation failed', errors: collectedErrors }
    };
  }
  ...
  if (typeof syncMultiUnitGalleryToCabinType === 'function') {
...
module.exports = {
  CABIN_ALLOWED_FIELDS,
  REQUIRED_CABIN_FIELDS,
  sanitizeCabinPayload,
  slugifyName,
  ensureUniqueCabinTypeSlug,
  mapUnitForResponse,
  fetchUnitsForCabinType,
  hasActiveBookings,
  canSwitchInventoryMode,
  buildCabinResponse,
  syncUnits,
